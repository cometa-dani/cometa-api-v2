...
@Service()
export class FiendshipRepository {
  ...
  async getLatestFriendsWithPagination(
  ...
  async sentFriendship(targetUserId: number, loggedInUserID: number) {
  ...
  async acceptFriendship(targetUserID: number, loggedInUser: User) {
    const friendshipExists = await this._prisma.friendship.findFirst({
      where: {
        OR: [
  ...
  async resetInvitation(targetUserID: number, loggedInUser: User) {
    const friendshipExists = await this._prisma.friendship.findFirst({
      where: {
        OR: [
          { senderId: targetUserID, receiverId: loggedInUser.id },
  ...
  async deleteFriendship(tagetUserID: number, loggedInUserID: number) {
    const friendshipExists = await this._prisma.friendship.findFirst({
      where: {
        OR: [
